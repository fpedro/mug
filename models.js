  ...
  updateRoundReward: function(username, idSession, round){
    ...
    for(i=0; i<pp.length; i++){
      ...
      if(numberAcceptorsProposalP>=sessionRule){
       ...
       //without name
       othersReport = othersReport.concat({id: "your opponent number "+(i+1)+" ", offer: (Math.round(parseFloat(pp[i])/(groupSize-1) * 100) / 100), acceptances: numberAcceptorsProposalP});
       }
       else{
	 //with name
       ...
       //without name
       othersReport = othersReport.concat({id: "your opponent number "+(i+1)+" ", offer: 0, acceptances: numberAcceptorsProposalP});
